 <?php
    ...
    if (isset($_POST['confirm_item'])) {
        ...
        $conexao->query("UPDATE OrcProdutos SET OrcStatus = 'Finalizado', OrcDataEntrega = CURRENT_DATE WHERE OrcId = $id");
    }
    ...
    if (isset($_POST['busca-btn'])) {
        ...
        $sql = "SELECT OrcId, OrcNome, OrcDescricao, OrcDataInicio, OrcPreco, OrcTipo,
            ...
            AND OrcStatus IN ('Entrega', 'Finalizado') ORDER BY OrcDataInicio DESC, OrcId DESC";
    ...
    if (!isset($_POST['busca-btn'])) {
    $sql = "SELECT OrcId, OrcNome, OrcDescricao, OrcDataProdInicio, OrcPreco, OrcTipo, OrcStatus, CliNome, CliEndereco, CliCidade, CliEmail, CliTelefone, CliCelular FROM OrcProdutos JOIN CliClientes ON OrcCliente = CliId WHERE OrcStatus IN ('Entrega','Finalizado') ORDER BY OrcStatus ASC, OrcId DESC";
 ...
 <body>
     <div id="dashboard" class="dashboard">
         <header class="dashboard-header">
             <h1><i class="fas fa-boxes"></i>Entrega</h1>
                 ...
                 <div class="sales-table-container">
                     ...
                     <table class="sales-table">
                         ...
                         <tbody>
                             ...
                             <?php while($row = $result->fetch_assoc()): $isFinalizado = ($row['OrcStatus'] === 'Finalizado'); ?>
                             <tr class="sale-row" data-sale-id="<?php echo $row['OrcId']; ?>"
                                 <?php if ($isFinalizado) echo 'style="background-color:#d6d6d6; color:#333;"'; ?>>
                                 <td><?php echo htmlspecialchars($row['OrcNome']); ?></td>
                                 <td>R$ <?php echo number_format($row['OrcPreco'], 2, ',', '.'); ?></td>
                                 <td><?php echo htmlspecialchars($row['OrcStatus']); ?></td>
                                 <td><?php echo htmlspecialchars($row['CliNome']); ?></td>
                                 <td>
                                     <!-- Formulário para apagar o item -->
                                     <?php if (!$isFinalizado): ?>
                                     <form method="post"
                                         onsubmit="return confirm('Tem certeza que deseja finalizar essa Entrega?');"
                                     ...
                                     <form method="post"
                                         onsubmit="return confirm('Tem certeza que deseja deletar esta Entrega?');"
                                         style="display:inline;">
