 <?php
    ...
    if (isset($_POST['confirm_item'])) {
        ...
        $conexao->query("UPDATE OrcProdutos SET OrcStatus = 'Entrega', OrcDataProdFim = CURRENT_DATE WHERE OrcId = $id");
    }
    ...
    if (isset($_POST['busca-btn'])) {
        ...
        $sql = "SELECT OrcId, OrcNome, OrcDescricao, OrcDataInicio, OrcPreco, OrcTipo,
            ...
            AND OrcStatus = 'Produção' ORDER BY OrcDataInicio DESC, OrcId DESC";
    ...
    if (!isset($_POST['busca-btn'])) {
    $sql = "SELECT OrcId, OrcNome, OrcDescricao, OrcDataProdInicio, OrcPreco, OrcTipo, CliNome, CliEndereco, CliCidade, CliEmail, CliTelefone, CliCelular FROM OrcProdutos JOIN CliClientes ON OrcCliente = CliId WHERE OrcStatus = 'Produção' ORDER BY OrcDataProdInicio DESC, OrcId DESC";
 ...
 <body>
     <div id="dashboard" class="dashboard">
         <header class="dashboard-header">
             <h1><i class="fas fa-cogs"></i>Produção</h1>
                     ...
                     <table class="sales-table">
                         ...
                         <tbody>
                             ...
                             <tr class="sale-row" data-sale-id="<?php echo $row['OrcId']; ?>">
                                 ...
                                 <td><?php echo date('d/m/Y', strtotime($row['OrcDataProdInicio'])); ?></td>
                                 <td><?php echo htmlspecialchars($row['CliNome']); ?></td>
                                 <td>
                                     <!-- Formulário para apagar o item -->
                                     <form method="post"
                                         onsubmit="return confirm('Tem certeza que deseja encerrar esse processo Produção?');"
                                     ...
                                     <form method="post"
                                         onsubmit="return confirm('Tem certeza que deseja deletar este processo de Produção ?');"
                                         style="display:inline;">
